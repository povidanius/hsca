...
def hcsa1(X, Y, d, lamda):
	...
	for i in range(d):
		...
		LLf = Lf - np.diag(np.diag(Lf));

		M1 = X*(LLf + (A*LLf*A - np.sum(np.sum(LLf))*np.eye(m))/((m-1)*(m-2)) - (LLf*A + A*LLf - 2*np.diag(np.diag(LLf*A)))/(m-2))*X.getH() + lamda*np.eye(M1.shape[0]) 
